...
use App\Models\model_fifo;
use CodeIgniter\Model;

class ob_keluar extends BaseController
{
    protected $model_fifo;
    public function __construct()
    {
        $this->model_fifo = new Model_fifo();
    }
    ...
    public function simpan2()
    {
        ...
        if ($hasil < 1) {
            ...
            foreach ($querydumpdet as $a) {
                ...
                // FIFO 
                session()->set(['kekurangan_keluar' => $jml_keluar]);

                $kondisi1 = ['kode_obat' => $kd_obat];
                $kondisi2 = ['sisa > ' => 0];

                $get_data_fifo = $this->model_fifo->get_fifo_data($kondisi1, $kondisi2);

                foreach ($get_data_fifo as $fifo) {
                    $sisa_akhir = $fifo['sisa'] - session()->get('kekurangan_keluar');
                    $kode_detail = $fifo['kode_detail'];
                    $kondisi_update_fifo = ['kode_detail' => $kode_detail];
                    if ($sisa_akhir < 0) {
                        $keluar = $fifo['keluar'] + $fifo['sisa'];
                        $kekurangan = $sisa_akhir * (-1);
                        $sisa = 0;
                        // update data fifo, keluar = $jml_keluar, sisa = 0
                        $data = [
                            'keluar' => $keluar,
                            'sisa' => $sisa,
                        ];
                        // eksekusi update
                        $this->model_fifo->update_fifo_data($data, $kondisi_update_fifo);
                        // set session sisa, $sisa_akhir x (-1)
                        session()->set(['kekurangan_keluar' => $kekurangan]);
                    } elseif ($sisa_akhir >= 0) {
                        $keluar = $fifo['keluar'] + session()->get('kekurangan_keluar');
                        $kekurangan = 0;
                        $sisa = $sisa_akhir;
                        // update data fifo, keluar = $jml_keluar, sisa = 0
                        $data = [
